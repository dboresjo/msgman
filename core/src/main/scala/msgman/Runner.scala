...
object Runner {
  ...
  private def resolve(config: Options, cwd: File, home: File, etc: File): Resolved = {
    ...
    Resolved(
      ...
      require = if (config.require.nonEmpty) config.require else csv("require"),
      priority = if (config.priority.nonEmpty) config.priority else csv("priority-keys")
    )
  }
  ...
  private def validate(resolved: Resolved): Option[String] =
    if (!Cli.isIsoCode(resolved.master)) Some(s"master must be a 2-letter ISO country code: ${resolved.master}")
    else if (!resolved.require.forall(Cli.isIsoCode))
  ...
  private def resolveTranslation(
    ...
    if (!config.translate) Right(None)
  ...
  private def runFormat(
    ...
    resolveTranslation(config, cwd, providers, home, etc, env) match {
      ...
      case Right(translation) =>
        ...
        val log: String => Unit = if (config.verbose) (msg: String) => out.println(s"msgman: $msg") else _ => ()
  ...
  private def finishFormat(
    ...
    val extraKeysByCode = extra.groupBy(_.languageCode).map { case (code, es) => code -> es.map(_.key).toSet }
    ...
    if (translationResults.values.exists(_._2.nonEmpty)) ExitCode.TranslationFailure else ExitCode.Success
  }
